...
- **Domínio + portas no installer**: **Superado pelo Sprint 22** — `scripts/install.sh` não pede mais domínio (nem email do Let's Encrypt) antes de subir a stack; detecta o IP público sozinho e o painel fica acessível via HTTP puro nesse IP desde o primeiro boot. Domínio/HTTPS passam a ser configurados de dentro do painel (Settings), não no installer. Portas **80 e 443** continuam precisando estar acessíveis publicamente na VPS (bootstrap HTTP no 80, desafio HTTP-01 do Let's Encrypt + HTTPS quando um domínio for configurado depois); o installer continua: (a) checando se as portas já estão ocupadas antes de subir a stack; (b) detectando `ufw` ativo e oferecendo abrir 80/443 automaticamente (com confirmação); (c) avisando no output final que o domínio se configura depois, em Settings.
