...
export default function DomainsPage() {
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-7">
    ...
    <Card className="py-0"><CardContent className="px-0">{mockDomains.length ? <Table><TableHeader><TableRow className="hover:bg-transparent"><TableHead className="pl-5">Hostname</TableHead><TableHead>Application</TableHead><TableHead>Type</TableHead><TableHead className="pr-5">SSL</TableHead></TableRow></TableHeader><TableBody>{mockDomains.map((domain) => { const app = mockApplications.find((item) => item.id === domain.applicationId); return <TableRow key={domain.id}><TableCell className="pl-5"><span className="flex items-center gap-2 font-medium"><Globe2 className="size-4 text-muted-foreground" />{domain.hostname}</span></TableCell><TableCell className="text-muted-foreground">{app?.name ?? "Unknown app"}</TableCell><TableCell>{domain.isPrimary ? <Badge variant="outline">Primary</Badge> : <span className="text-sm text-muted-foreground">Alias</span>}</TableCell><TableCell className="pr-5"><Badge variant="outline" className={cn("capitalize", sslClasses[domain.sslStatus])}>{domain.sslStatus}</Badge></TableCell></TableRow>; })}</TableBody></Table> : <EmptyState icon={Globe2} title="No domains yet" description="Attach a hostname to an application to provision SSL automatically." action={<Button onClick={() => setOpen(true)}><Plus className="size-4" />Add Domain</Button>} />}</CardContent></Card>
